...
fn connect() -> redis::Connection {
    env::set_var("REDIS_HOSTNAME", "localhost:6379");
    //format - host:port
    let redis_host_name =
...
fn basics() {
    ...
    let mut client = basics_client();
    let _: () = client.set_ex("year", 2022, 20).expect("set error");
    let year:i32 = client.get("year").unwrap();
    println!("year:{:?}", year);
